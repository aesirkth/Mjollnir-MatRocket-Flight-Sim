...
figlist{end+1} = figure("name", "drag coefficients");
...
a = {ax15x,ax15y,ax15z};
for index = 1:numel(a)
a{index}.NextPlot = "add";
ylabel(a{index}, "drag-coefficient")
if notick; xticks(a{index}, []); yticks(a{index}, []); end
end
...
plot(ax10sht, rocket_historian.t, mag(rocket_historian.forces.chute_drag.vector), "Color", color)
...
plot(ax15x, rocket_historian.t, rocket_historian.aerodynamics.pressure_coefficient(1,:), "Color", color);
plot(ax15y, rocket_historian.t, rocket_historian.aerodynamics.pressure_coefficient(2,:), "Color", color);
plot(ax15z, rocket_historian.t, rocket_historian.aerodynamics.pressure_coefficient(3,:), "Color", color);
...
if ~isfolder(dirname)
mkdir(dirname)
copyfile("latest_sim.mat", dirname+"\"+"sim.mat")
copyfile("Output\flight.csv", dirname+"\"+"flight.csv")
end
