...
% Plot range boundary
range = shaperead("Parameter_studies/Geodata/range.shp");
crs = projcrs(3006); % SWEREF99 TM

for k = 1:numel(range)
    [latR, lonR] = projinv(crs, range(k).X, range(k).Y);
    geoplot(gx, latR, lonR, "w-", "LineWidth", 1);
end
...
lon0 = 19.329861;
alt0 = 0;
geoscatter(gx, lat0, lon0, 30, 'g', 'filled');
text(gx, lat0, lon0, ' Launch site', 'VerticalAlignment','bottom', 'HorizontalAlignment','left', 'Color','w');
hold on;
...
geoplot(gx, latA, longA,'--', LineWidth=1);
text(gx, 67.9101134, 21.1987985, ' Impact zone A', 'VerticalAlignment','bottom', 'HorizontalAlignment','left', 'Color','w');
